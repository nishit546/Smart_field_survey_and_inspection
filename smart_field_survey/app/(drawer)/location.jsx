...
export default function LocationScreen() {
    ...
    const [latitude, setLatitude] = useState(tempSurvey.latitude);
    const [longitude, setLongitude] = useState(tempSurvey.longitude);
    const [accuracy, setAccuracy] = useState(tempSurvey.locationAccuracy);
    useEffect(() => {
        checkPermissions();
    }, []);
    ...
    const fetchLocation = async () => {
        try {
            ...
            Alert.alert('GPS Lock Error', 'Could not obtain a GPS signal. Please check if location services are enabled on your device.');
        }
        finally {
            setIsLoading(false);
        }
    ...
    const handleCopyLocation = async () => {
        ...
        try {
            ...
            Alert.alert('Location Copied', 'The GPS coordinates have been successfully copied to your clipboard.', [{ text: 'OK' }]);
        }
        catch (error) {
    ...
    return (<View style={[styles.mainContainer, { backgroundColor: isDark ? '#151718' : '#f8fafc' }]}>
      ...
      {permissionStatus === null && (<View style={styles.centerContainer}>
          <ActivityIndicator size="large" color="#6d28d9"/>
          <Text style={styles.loadingText}>Checking location permission status...</Text>
        </View>)}

      {/* Renders if permissions are denied */}
      {permissionStatus !== null && !isGranted && (<View style={styles.centerContainer}>
          <View style={styles.card}>
            ...
            <Pressable style={[styles.primaryBtn, { backgroundColor: '#6d28d9' }]} onPress={handleRequestPermission}>
              <Text style={styles.primaryBtnText}>Grant Permission</Text>
            </Pressable>

            {permissionStatus === Location.PermissionStatus.DENIED && (<Pressable style={styles.secondaryBtn} onPress={() => Linking.openSettings()}>
                <Text style={styles.secondaryBtnText}>Open System Settings</Text>
              </Pressable>)}

            <Pressable style={styles.backBtn} onPress={handleSaveAndExit}>
      ...
      {permissionStatus !== null && isGranted && (<ScrollView contentContainerStyle={styles.scrollContainer}>
          ...
          <View style={styles.actionPanel}>
            <Pressable style={[styles.actionBtn, styles.refreshBtn, { backgroundColor: isDark ? '#1e293b' : '#ffffff' }]} onPress={fetchLocation} disabled={isLoading}>
            ...
            <Pressable style={[styles.actionBtn, styles.copyBtn, { backgroundColor: '#6d28d9' }]} onPress={handleCopyLocation} disabled={latitude === undefined}>
          ...
          {/* Save Button */}
          <Pressable style={[styles.saveBtn, { backgroundColor: '#10b981' }]} onPress={handleSaveAndExit}>
