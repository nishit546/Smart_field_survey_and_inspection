...
export default function HistoryScreen() {
    ...
    const handleDeleteSurvey = (id, siteName) => {
        Alert.alert('Delete Survey', `Are you sure you want to permanently delete the inspection record for "${siteName}"?`, [
    ...
    const filteredSurveys = surveys.filter((survey) => {
        ...
        const matchesSearch = survey.siteName.toLowerCase().includes(query) ||
    ...
    const renderItem = ({ item }) => {
        ...
        const priorityBg = item.priority === 'High' ? '#fee2e2' :
        ...
        <View style={styles.cardFooter}>
          ...
          <View style={styles.attachmentsRow}>
            {hasPhoto && (<View style={styles.attachmentIcon}>
                <Ionicons name="camera" size={14} color="#6d28d9"/>
              </View>)}
            {hasLocation && (<View style={styles.attachmentIcon}>
                <Ionicons name="location" size={14} color="#6d28d9"/>
              </View>)}
            {hasContact && (<View style={styles.attachmentIcon}>
                <Ionicons name="person" size={14} color="#6d28d9"/>
              </View>)}
            {hasNotes && (<View style={styles.attachmentIcon}>
                <Ionicons name="clipboard" size={14} color="#6d28d9"/>
              </View>)}
          </View>

          {/* Inline Delete Trigger */}
          <Pressable style={styles.deleteButton} onPress={() => handleDeleteSurvey(item.id, item.siteName)}>
    ...
    const renderEmptyState = () => (<View style={[styles.emptyCard, { backgroundColor: isDark ? '#1e293b' : '#ffffff' }]}>
      ...
      {!searchQuery && selectedPriority === 'All' && (<Pressable style={styles.createBtn} onPress={() => router.push('/(drawer)/(tabs)/new-survey')}>
    ...
    return (<View style={[styles.mainContainer, { backgroundColor: isDark ? '#151718' : '#f8fafc' }]}>
      ...
      <View style={[styles.filterHeader, { backgroundColor: isDark ? '#1e293b' : '#ffffff', borderBottomColor: isDark ? '#334155' : '#e5e7eb' }]}>
        ...
        <View style={[styles.searchBar, { backgroundColor: isDark ? '#0f172a' : '#f1f5f9' }]}>
          ...
          <TextInput style={[styles.searchInput, { color: isDark ? '#f1f5f9' : '#1e293b' }]} value={searchQuery} placeholder="Search by site or client..." placeholderTextColor="#94a3b8" onChangeText={setSearchQuery}/>
        ...
        <View style={styles.chipsRow}>
          {['All', 'High', 'Medium', 'Low'].map((priorityOption) => {
            ...
            const chipColor = priorityOption === 'High' ? '#ef4444' :
                priorityOption === 'Medium' ? '#d97706' :
                    priorityOption === 'Low' ? '#10b981' : '#6d28d9';
            return (<Pressable key={priorityOption} style={[
                    styles.filterChip,
                    {
      ...
      <FlatList data={filteredSurveys} keyExtractor={(item) => item.id} renderItem={renderItem} ListEmptyComponent={renderEmptyState} contentContainerStyle={styles.listContainer} refreshing={isRefreshing} onRefresh={handleRefresh}/>
    </View>);
}
