...
import { StyleSheet, View, Text, FlatList, TextInput, Pressable, ActivityIndicator, RefreshControl, Alert, } from 'react-native';
...
export default function ContactsScreen() {
    ...
    const handleCopyNumber = async (number, name) => {
        try {
            ...
            Alert.alert('Number Copied', `Phone number for ${name} has been copied to your clipboard.`, [{ text: 'OK' }]);
        }
        catch (error) {
    ...
    const renderItem = ({ item }) => {
        ...
        <Pressable style={styles.contactPressable} onPress={() => handleSelectContact(item)}>
          ...
          <View style={styles.contactDetails}>
            ...
            {item.phoneNumber ? (<Text style={styles.contactPhone}>{item.phoneNumber}</Text>) : (<Text style={styles.noNumberText}>No Number</Text>)}
          </View>
        </Pressable>

        {item.phoneNumber && (<Pressable style={styles.copyButton} onPress={() => handleCopyNumber(item.phoneNumber, item.name)}>
    ...
    return (<View style={[styles.mainContainer, { backgroundColor: isDark ? '#151718' : '#f8fafc' }]}>
      ...
      <View style={[styles.searchHeader, { backgroundColor: isDark ? '#1e293b' : '#ffffff', borderBottomColor: isDark ? '#334155' : '#e5e7eb' }]}>
        <View style={[styles.searchBar, { backgroundColor: isDark ? '#0f172a' : '#f1f5f9' }]}>
          ...
          <TextInput style={[styles.searchInput, { color: isDark ? '#f1f5f9' : '#1e293b' }]} value={searchQuery} placeholder="Search clients by name or phone..." placeholderTextColor="#94a3b8" onChangeText={setSearchQuery}/>
          {searchQuery ? (<Pressable onPress={() => setSearchQuery('')} style={styles.clearBtn}>
        ...
        <View style={styles.metaRow}>
          ...
          {permissionStatus !== Contacts.PermissionStatus.GRANTED && (<Text style={styles.mockBadge}>Running Sandbox Mode</Text>)}
        </View>
      </View>

      {/* Renders Loader */}
      {isLoading ? (<View style={styles.centerContainer}>
          <ActivityIndicator size="large" color="#6d28d9"/>
          <Text style={styles.loadingText}>Fetching clients...</Text>
        </View>) : (<FlatList data={filteredContacts} keyExtractor={(item) => item.id} renderItem={renderItem} ListEmptyComponent={renderEmptyState} contentContainerStyle={styles.listContainer} refreshControl={<RefreshControl refreshing={isRefreshing} onRefresh={handleRefresh} tintColor="#6d28d9" colors={['#6d28d9']}/>}/>)}

      {/* Permissions Helper Alert if denied */}
      {permissionStatus !== null && permissionStatus !== Contacts.PermissionStatus.GRANTED && (<View style={[styles.warningBanner, { backgroundColor: isDark ? '#78350f' : '#fef3c7' }]}>
