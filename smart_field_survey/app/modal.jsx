...
export default function ModalScreen() {
    ...
    if (!survey) {
        return (<View style={[styles.errorContainer, { backgroundColor: isDark ? '#151718' : '#f8fafc' }]}>
    ...
    const handleSubmit = () => {
        ...
        if (savedSurvey) {
            Alert.alert('Survey Submitted', 'Your field inspection survey has been successfully saved to history.', [
                {
                    text: 'OK',
                    onPress: () => {
    ...
    const handleDelete = () => {
        if (isPreview)
            return;
        Alert.alert('Delete Survey', 'Are you sure you want to permanently delete this survey from history?', [
    ...
    const priorityColor = survey.priority === 'High' ? '#ef4444' :
        survey.priority === 'Medium' ? '#d97706' : '#10b981';
    const priorityBg = survey.priority === 'High' ? '#fee2e2' :
        survey.priority === 'Medium' ? '#fef3c7' : '#d1fae5';
    return (<View style={[styles.mainContainer, { backgroundColor: isDark ? '#151718' : '#f8fafc', paddingBottom: insets.bottom }]}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.card, { backgroundColor: isDark ? '#1e293b' : '#ffffff' }]}>
          ...
          <View style={styles.metaRow}>
            ...
            {!isPreview && (<View style={styles.metaItem}>
        ...
        {survey.photoUri ? (<View style={[styles.card, { backgroundColor: isDark ? '#1e293b' : '#ffffff' }]}>
        ...
        {survey.latitude !== undefined && survey.longitude !== undefined ? (<View style={[styles.card, { backgroundColor: isDark ? '#1e293b' : '#ffffff' }]}>
            ...
            {survey.locationAccuracy !== undefined ? (<View style={styles.accuracyFooter}>
        ...
        {/* Client Contact Info */}
        {survey.contactName ? (<View style={[styles.card, { backgroundColor: isDark ? '#1e293b' : '#ffffff' }]}>
        ...
        {survey.notes ? (<View style={[styles.card, { backgroundColor: isDark ? '#1e293b' : '#ffffff' }]}>
